...
use sui_types::storage::{OneShotSharedInMemoryStore, ReadStore, WriteStore};
use tracing::info;
...
pub async fn verify_archive_with_genesis_config(
    ...
    let mut store = OneShotSharedInMemoryStore::default();
    ...
    <S as ReadStore>::Error: std::error::Error,
{
    ...
    let progress_bar = if interactive {
        ...
        tokio::spawn(async move {
            loop {
                let curr_latest_checkpoint = cloned_store
                    .get_highest_synced_checkpoint()
                    .unwrap()
                    .sequence_number;
